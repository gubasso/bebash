...
@test "edit --new scaffolds data function and opens editor" {
...
EOS
  ...
  [ "$status" -eq 0 ]
  [ ! -s "$TMPDIR/stdout" ]
  [ "$(cat "$EDITOR_SEEN")" = "$XDG_DATA_HOME/bebash/functions/new_fn.bash" ]
  grep -q 'new_fn()' "$XDG_DATA_HOME/bebash/functions/new_fn.bash"
}
