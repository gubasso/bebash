...
`pick_multi`, `require_tty`, and `require_gui`. `BEBASH_UI=auto|tty|gui|none`
...
input and `notify-send` for feedback. `none` is a silent mode: `interact::msg`
emits nothing (no `__ui_*` line, no notification), so a caller can run any
command purely as a feedback-free exit-code probe — the general form of the
"redirect `>/dev/null` doesn't silence `notify-send`" problem. `none` is for
non-prompting probes; prompts are out of its contract.
...
- Good: `BEBASH_UI=none` gives callers a clean way to reuse a command as a silent
  probe without the command needing a bespoke `--quiet` flag.
